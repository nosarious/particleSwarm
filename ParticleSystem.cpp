...
void ParticleSystem::setup(int width, int height, int k, int red, int green, int blue) { //set up variables for particle
	this->width = width;
	this->height = height;
	this->k = k;
	this->red = red;
...
#ifdef DRAW_FORCES
					ofSetColor(curParticle.red, curParticle.green, curParticle.blue, 70);
...
void ParticleSystem::draw() {
	...
	glBegin(GL_POINTS);
	for(int i = 0; i < n; i++){
				ofSetColor(particles[i].red, particles[i].green, particles[i].blue, 70);
				particles[i].draw();
	}
	glEnd();
}